...
ARTICLE_MONITOR_LOG_LEVEL = os.getenv('ARTICLE_MONITOR_LOG_LEVEL', 'INFO').strip().upper() or 'INFO'
ARTICLE_MONITOR_LOG_TO_FILE = os.getenv('ARTICLE_MONITOR_LOG_TO_FILE', '0').strip()
ARTICLE_MONITOR_LOG_FILE = os.getenv('ARTICLE_MONITOR_LOG_FILE', '').strip()
ARTICLE_MONITOR_LOG_FORMAT_CONSOLE = os.getenv('ARTICLE_MONITOR_LOG_FORMAT_CONSOLE', 'text').strip().lower()
ARTICLE_MONITOR_LOG_FORMAT_FILE = os.getenv('ARTICLE_MONITOR_LOG_FORMAT_FILE', 'json').strip().lower()
ARTICLE_MONITOR_LOG_MAX_BYTES = os.getenv('ARTICLE_MONITOR_LOG_MAX_BYTES', str(50 * 1024 * 1024)).strip()
ARTICLE_MONITOR_LOG_BACKUP_COUNT = os.getenv('ARTICLE_MONITOR_LOG_BACKUP_COUNT', '10').strip()
ARTICLE_MONITOR_LOG_QUEUE_ENABLED = os.getenv('ARTICLE_MONITOR_LOG_QUEUE_ENABLED', '1').strip()
ARTICLE_MONITOR_LOG_QUEUE_MAXSIZE = os.getenv('ARTICLE_MONITOR_LOG_QUEUE_MAXSIZE', '10000').strip()
...
# 批次结果重试（用于 crawl_urls_for_results / Bitable sync）
# 在首轮之后，最多额外重试 N 轮（只重试可重试失败项）
RESULT_RETRY_EXTRA_PASSES = max(0, int(os.getenv('RESULT_RETRY_EXTRA_PASSES', '1')))
...
# ==================== Proxy / Tor（免費動態 proxy） ====================
# Tor socks5 用於 blocked 時的最後一層 fallback（預設關閉）
TOR_ENABLED = os.getenv('TOR_ENABLED', 'True').lower() in ('true', '1', 'yes')
TOR_SOCKS5_URL = os.getenv('TOR_SOCKS5_URL', '').strip()
# 只在偵測 blocked/captcha/挑戰頁時才走 Tor（推薦）
TOR_ON_BLOCKED_ONLY = os.getenv('TOR_ON_BLOCKED_ONLY', 'True').lower() in ('true', '1', 'yes')
# blocked 後最多用 Tor 重試幾次（MVP 建議 1）
TOR_BLOCKED_RETRY_MAX = max(0, int(os.getenv('TOR_BLOCKED_RETRY_MAX', '1')))
...
if ALLOWED_PLATFORMS_ENV:
    ...
    # 默认白名单：只允许常用平台
    ALLOWED_PLATFORMS = ['juejin', 'csdn', 'cnblog', '51cto', 'segmentfault', 'jinshu', 'MBB', 'elecfans', 'sohu', 'eefocus', 'eet_china']
