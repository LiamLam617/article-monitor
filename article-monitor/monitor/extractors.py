...
from .config import (
    ...
    TOR_SOCKS5_URL,
    TOR_ON_BLOCKED_ONLY,
    TOR_BLOCKED_RETRY_MAX,
)
from .anti_scraping import get_anti_scraping_manager, AntiScrapingManager
...
def _log_extract_event(event: str, **fields):
    ...
    logger.info(
        event, extra=bind_context_fields({"event": event, **normalized_fields})
    )
...
async def _crawl_with_shared(
    url: str, crawler: AsyncWebCrawler, crawler_config: CrawlerRunConfig
):
    """使用共享浏览器实例爬取页面（内部函数）
...
_TITLE_SUFFIX_PATTERNS = [
    re.compile(
        r"\s*[-|_–—]\s*(掘金|CSDN|博客园|51CTO|SegmentFault|简书|电子发烧友|与非网).*$",
        re.IGNORECASE,
    ),
...
async def extract_with_config(
    url: str, platform: str, crawler: Optional[AsyncWebCrawler] = None
) -> Optional[int]:
    """使用配置文件提取阅读数
...
async def extract_with_config_full(
    url: str, platform: str, crawler: Optional[AsyncWebCrawler] = None
) -> tuple:
    """使用配置文件提取阅读数和标题
    ...
    if ANTI_SCRAPING_ENABLED:
        ...
        base_crawler_config = manager.get_crawler_config(
            timeout=timeout, wait_for=wait_for
        )
...
async def extract_article_info(
    ...
    if platform and platform in PLATFORM_EXTRACTORS:
        result["read_count"], result["title"] = await extract_with_config_full(
            url, platform, crawler
        )
