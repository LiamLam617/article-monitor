...
def _get_semaphores_lock_for_current_loop() -> asyncio.Lock:
    ...
    with _loop_locks_guard:
        for closed_loop in [
            loop for loop in _domain_semaphores_lock_by_loop if loop.is_closed()
        ]:
...
def _get_rate_limit_lock_for_current_loop() -> asyncio.Lock:
    ...
    with _loop_locks_guard:
        for closed_loop in [
            loop for loop in _domain_rate_limit_lock_by_loop if loop.is_closed()
        ]:
...
async def crawl_article_with_retry(
    article: dict,
    crawler=None,
    semaphore=None,
...
def _get_max_retries_for_category(error_category: ErrorCategory) -> int:
    ...
    if (
        ...
        or error_category == ErrorCategory.UNKNOWN
    ):
...
async def _crawl_with_retry(
    article: dict, crawler=None, max_retries: int = 3, mark_error_on_fail: bool = True
) -> bool:
    """带重试机制的爬取逻辑（同时更新标题）
    ...
    context_token = set_log_context(
        article_id=article_id, url=url, platform=article.get("site")
    )
        ...
        for attempt in range(max_retries + 1):  # 0到max_retries，共max_retries+1次尝试
            try:
                ...
                if count is None:
                    ...
                    category_max_retries = _get_max_retries_for_category(
                        last_error_category
                    )
                    effective_max_retries = min(max_retries, category_max_retries)

                    if attempt <= effective_max_retries:
                        logger.info("提取失败，将重试")
                        last_error = parse_error
                        continue
                    logger.warning(
                        f"❌ 无法提取阅读数: {url} (已重试 {effective_max_retries} 次)"
                    )
                ...
                if is_retryable and attempt <= effective_max_retries:
                    logger.warning(
                        f"⚠️  可重试错误 [{last_error_category.value}] (尝试 {attempt + 1}/{effective_max_retries + 1}): {url} - {str(e)[:100]}"
                    )
                    continue
                ...
                if last_error_category == ErrorCategory.NETWORK:
                    logger.error(
                        f"⏱️  网络错误 {url}: {error_msg} (已重试 {attempt} 次)"
                    )
                elif last_error_category == ErrorCategory.PARSE:
                    logger.error(
                        f"🔍 解析错误 {url}: {error_msg} (已重试 {attempt} 次)"
                    )
                elif last_error_category == ErrorCategory.SSL:
                    logger.error(f"🔒 SSL错误 {url}: {error_msg} (已重试 {attempt} 次)")
                else:
                    logger.error(
                        f"❌ 爬取失败 {url}: {error_msg} (已重试 {attempt} 次)"
                    )

                # 更新状态为失败（根据参数决定是否标记）
                # 第一轮不标记ERROR（等待集中重试），第二轮才标记
                if mark_error_on_fail and (
                    not is_retryable or attempt >= effective_max_retries
                ):
...
async def crawl_all_articles():
    ...
    try:
        ...
        logger.info("开始爬取 %s 篇文章", len(articles))
        ...
        if use_shared_crawler:
            try:
                ...
                logger.warning(f"无法创建共享浏览器实例: {e}")
                shared_crawler = None
            ...
            async def _do_retry_with_pool(article: dict, index: int) -> bool:
                ...
                try:
                    ...
                    if not crawler:
                        ...
                        use_pool = False
                    try:
                        result = await _crawl_with_retry(
                            article, crawler, CRAWL_MAX_RETRIES, mark_error_on_fail=True
                        )
                        with _crawl_progress_lock:
